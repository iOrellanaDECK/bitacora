    ...
    <article class="rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
                ...
                <h1 class="mt-3 text-2xl font-bold text-slate-900">{{ $proyecto->titulo }}</h1>

                {{-- Stack --}}
                <div class="mt-3 flex flex-wrap gap-2">
                    @foreach (explode(',', $proyecto->stack) as $tech)
                        <span class="inline-block rounded-md bg-indigo-50 text-indigo-700 px-2.5 py-1 text-xs font-medium">
                            {{ trim($tech) }}
                        </span>
                    @endforeach
                </div>
        ...
        <div class="mt-8 border-t border-slate-100 pt-6">
            <h2 class="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3">Resumen</h2>
            <p class="text-slate-700 leading-relaxed whitespace-pre-line">{{ $proyecto->resumen }}</p>
        </div>

        {{-- Meta --}}
        <div class="mt-8 border-t border-slate-100 pt-6 flex gap-6 text-xs text-slate-400">
